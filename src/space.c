...
static void space_body_update(Space *space,Body* body)
{
	...
	//slog("%s: Position %.2f, %.2f, %.2f", body->classtype, body->position.x, body->position.y, body->position.z);
	//slog("%s: Absolute Bounds %.2f, %.2f, %.2f, %.2f, %.2f, %.2f,", body->classtype, a.x, a.y, a.z, a.w, a.h, a.d);
    ...
    for (it = space->bodylist;it != NULL;it = g_list_next(it))
    {
		...
		//slog("%s: %.2f, %.2f, %.2f", other->classtype, other->position.x, other->position.y, other->position.z);
		//slog("%s: %.2f, %.2f, %.2f, %.2f, %.2f, %.2f,", other->classtype, b.x, b.y, b.z, b.w, b.h, b.d);
